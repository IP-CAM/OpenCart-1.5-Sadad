...
protected function index() {
          $this->language->load('payment/sadad');
             $sadad_live = $this->config->get('sadad_live');
    ...
    $this->load->model('checkout/order');
          ...
          if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/sadad.tpl')) {
               $this->template = $this->config->get('config_template') . '/template/payment/sadad.tpl';
          }else {
               $this->template = 'default/template/payment/sadad.tpl';
          }    
                   $this->render();
  ...
  public function callback() {
     ...
     $dd=$this->load->model('checkout/order');
             $sadad_live = $this->config->get('sadad_live');
      ...
      if (isset($_SESSION['secret_key'])) {
          ...
          $paytabs_merchant = $this->config->get('sadad_merchant');

          $site = $_SESSION['site'];
            $request_param =array('secret_key'=>$secret_key,'merchant_email'=>$this->config->get('sadad_merchant'),'payment_reference'=>$_POST['payment_reference']);
         ...
         $response = $object->response_code;
               ...
               if ($response == 100) {
                    $this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('sadad_order_status_id'));
               ...
               //$this->load->language('checkout/failure');
                    ...
                    $this->template = $this->config->get('config_template') . '/template/payment/sadad_error.tpl';
               ...
               //$this->load->view($this->config->get('config_template') . '/template/payment/sadad_error.tpl', $data);

               //return  $this->response->setOutput($this->config->get('config_template') . '/template/payment/sadad_error.tpl', $data);
     ...
     public function send() {
         ...
         $total=$this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false);
          ...
          if ($_SERVER['SERVER_PORT']==443) {
             ...
             $payment_data = array(
               'merchant_email'        => $this->config->get('sadad_merchant'),
               'secret_key'           => $this->config->get('sadad_security'),
               ...
               'return_url'           => $this->url->link('payment/sadad/callback', 'order_id=' .$this->session->data['order_id'])

               
          );

              
            $_SESSION['secret_key'] = $this->config->get('sadad_security');
            $_SESSION['paytabs_merchant'] = $this->config->get('sadad_merchant');
